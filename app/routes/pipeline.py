...
def _run_pipeline(pdf_upload_id: str):
    try:
        # 1. Busca registro do upload
        rows = db_client.read("pdf_uploads", {"id": pdf_upload_id})
        if not rows:
            return
        upload = rows[0]

        # 2. Marca como processing
        db_client.update_pdf_status(pdf_upload_id, "processing")

        # 3. Download + extração de texto
        ...
        db_client.update_pdf_status(pdf_upload_id, "completed")

    except PDFScannedError as e:
        db_client.update_pdf_status(pdf_upload_id, "error", str(e))
    except PDFExtractionError as e:
        db_client.update_pdf_status(pdf_upload_id, "error", str(e))
    except Exception as e:
        db_client.update_pdf_status(pdf_upload_id, "error", f"Erro interno: {e}")


@router.post("/pipeline/process")
def process_pdf(request: ProcessRequest, background_tasks: BackgroundTasks):
    """Inicia o pipeline de processamento em background."""
...
def generate_analysis(request: GenerateAnalysisRequest):
    """Gera justificativas e peguinhas para questões (batch)."""
...
def extract_syllabus(request: ExtractSyllabusRequest):
    """Extrai conteúdo programático de um texto de edital."""
